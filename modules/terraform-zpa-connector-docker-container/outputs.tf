...
output "container_name" {
  ...
  value       = docker_container.zpa_service_edge_container[*].name
}

output "hostname" {
  description = "Hostname of the container."
  value       = docker_container.zpa_service_edge_container[*].hostname
}

output "working_dir" {
  description = "The working directory for commands to run in."
  value       = docker_container.zpa_service_edge_container[*].working_dir
}

output "restart" {
  description = "The restart policy for the container. Must be one of 'no', 'on-failure', 'always', 'unless-stopped'. Defaults to no"
  value       = docker_container.zpa_service_edge_container[*].restart
}

output "privileged" {
  description = "If true, the container runs in privileged mode."
  value       = docker_container.zpa_service_edge_container[*].privileged
}

output "network_mode" {
  description = "Network mode of the container."
  value       = docker_container.zpa_service_edge_container[*].network_mode
}

output "dns" {
  description = "DNS servers to use."
  value       = docker_container.zpa_service_edge_container[*].dns
}
...
output "entrypoint" {
  ...
  EOF
  value       = docker_container.zpa_service_edge_container[*].entrypoint
}
...
output "command" {
  ...
  EOF
  value       = docker_container.zpa_service_edge_container[*].command
}

output "ports" {
  description = "Publish a container's port(s) to the host."
  value       = docker_container.zpa_service_edge_container[*].ports
}

output "volumes" {
  description = "Spec for mounting volumes in the container."
  value       = docker_container.zpa_service_edge_container[*].volumes
}

output "healthcheck" {
  description = "A test to perform to check that the container is healthy"
  value       = docker_container.zpa_service_edge_container[*].healthcheck
}

output "environment" {
  description = "Environment variables to set in the form of KEY=VALUE, e.g. DEBUG=0"
  value       = docker_container.zpa_service_edge_container[*].env
}

output "devices" {
  description = "Bind devices to the container"
  value       = docker_container.zpa_service_edge_container[*].devices
}

output "capabilities" {
  description = "Add or drop certrain linux capabilities."
  value       = docker_container.zpa_service_edge_container[*].capabilities
}

output "networks_advanced" {
  description = "The networks the container is attached to"
  value       = docker_container.zpa_service_edge_container[*].networks_advanced
}
